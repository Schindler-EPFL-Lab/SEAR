...
This repo will be published soon.
...
```bash
python sear/scripts/train_ablation_model.py --.aggregator.type CAMERA_TOKEN --dataset-mode NO_TRAJECTORY_SPECIFIED
```

Ablation studies can run by using the other aggregator-types found in `sear/ablation_models/possible_aggregators.py`.

Models can be evaluated with `sear/scripts/ablation_vggt.py`
...
- [Orebro-to-be-renamed](https://to-be-updated.com)

We provide a [link](https://to-be-updated.com) to our training dataset.

The dataset must have the following format:
...
If one wishes to process the [Orebro](#processing-orebro) then they should Install ROS2.
...
1. One should install the official [mast3r repo](https://github.com/naver/mast3r) in the root folder of the project and build the CroCo following the official installation [guidelines](https://github.com/naver/mast3r?tab=readme-ov-file#installation):
...
1. There must be a data asset EvalOutputs_VGGTAblationStudies, in order to save the outputs
2. Please update the job_statuses.json with new jobs, their statuses and also specify job statuses of the finished experiments.
3. Run the script `src/rebel-pose/sear/scripts/missing_experiments/launch_eval_ablation_models.py`, it will launch two eval jobs (one for ThermoScenes and one for DifficultScenesVGGT) and save outputs in EvalOutputs_VGGTAblationStudies
4. After all the eval jobs are completed, one should launch `src/rebel-pose/azure_scripts/eval_using_multiple_predictions.py`, it will calculate the metrics based on the outputs stored in EvalOutputs_VGGTAblationStudies. I believe the execution of this job will be fast and it will not be preempted. It will output the metrics results in outputs/job_name.
5. Send me the outputs folder of the previous, and I will create a table + calculate std's
