...
from deep_image_matching.utils.loftr_roma_to_multiview import (
...
def run_matching(args):
    """
    Main processing function that performs image matching and reconstruction using
    ...
    # Initialize ImageMatcherForMatchAnything class
    matcher = CustomImageMatcher(config, checkpoint_path=Path(args["checkpoint_path"]))
    if config.general["matching_strategy"] == "custom_pairs":
        matcher.pair_file = (
            Path(config.general["pair_file"]).parent.parent / "custom_pairs.txt"
        )
    ...
    if not config.general["skip_reconstruction"]:
        reconst_opts = {}
        _ = dim.reconstruction.incremental_reconstruction(
            database_path=output_dir / "database.db",
            image_dir=imgs_dir,
