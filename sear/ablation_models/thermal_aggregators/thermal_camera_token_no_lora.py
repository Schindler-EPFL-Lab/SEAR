...
class ThermalAggregatorThermalCameraTokenNoLoRA(ThermalAggregatorBase):
    """
    An ablation study for the proposed ThermalVGGT method which does not use LoRA and
    ...
    KEEP_IN_STATE_DICT: set[str] = {"thermal_camera_token"}
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], *args, **kwargs) -> None:
        """
        Loads the `state_dict` of the model which must contain the state dict of the
