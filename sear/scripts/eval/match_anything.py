...
from sear.scripts.eval.match_anything_config import MatchAnythingEvalParameters
from sear.scripts.eval.read_colmap import colmap_to_json, find_best_reconstruction


class MatchAnythingChunkProcessor(ChunkProcessorBase):
    """A base class process a Chunk of images using MatchAnything"""

    def __init__(self, config: MatchAnythingEvalParameters) -> None:
        """Instantiates `MatchAnythingChunkProcessor` using parameters from `config`."""
        super().__init__()
        self._config = config

    def load_model(self) -> None:
        """The MatchAnything has an underlying model, but it is loaded later."""
        pass
    ...
    def process_chunk(
        ...
        """
        Runs MatchAnything on images from `chunk`. The cache files created during the
...
if __name__ == "__main__":
    params = tyro.cli(MatchAnythingEvalParameters)
    chunk_processor = MatchAnythingChunkProcessor(params)
