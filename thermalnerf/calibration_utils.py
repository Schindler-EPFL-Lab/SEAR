"""
This file was taken from the ThermalNeRF repository (called nerfstudio-thermal), the
location is "./nerfstudio/process_data/calibration_utils.py", the gist file (as for 15
December 2025)
https://raw.githubusercontent.com/yvette256/nerfstudio-thermal/refs/heads/main/nerfstudio/process_data/calibration_utils.py

Instead of downloading the official package the file is taken because of their
pyproject.toml which is incompatible with ours.

Although we do not maintain the, a few changes have been done:
    - The dataset is used with VGGT-processed images, which have different resolution in
      comparison to the original images (rgb and thermal). One needs to set the
      hyperparameter `params.minArea = 80` since both rgb and thermal circles are
      smaller than those in the original images.
    - Removed the first function `calibrate_rgb_thermal` because there should not be two
      functions with the same name.
    - Removed unused nerfstudio imports.
"""
